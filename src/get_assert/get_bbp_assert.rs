...
pub async fn get_assertions(
    ...
    // 检查文件是否存在
    let file_path = "identifiers.txt";
    let file_exists = Path::new(file_path).exists();
    // 打开文件以追加数据，如果文件不存在则创建文件
    ...
    for i in handle.iter() {
        ...
        match response.status() {
            reqwest::StatusCode::OK => {
                ...
                if let Some(nodes) =
                    v["data"]["team"]["structured_scopes_search"]["nodes"].as_array()
                {
                    for node in nodes {
